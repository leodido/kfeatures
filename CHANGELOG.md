...
- AGENTS "Git author identity" section rewritten to reference only the GitHub-issued noreply identity. The previous wording quoted the underlying private address verbatim as a teaching example for the GH007 rejection mechanism; that has been removed. The noreply form, the GH007 error message, and the recovery procedure are unchanged. An explicit instruction not to quote the rejected private address anywhere in the repo, commit messages, or PR descriptions has been added.
